...
namespace PDFLibNet {
	...
	protected:	
		...
		{	
			...
			if(_gchRenderNotifyFinished.IsAllocated)
				_gchRenderNotifyFinished.Free();
			if(_gchRenderFinished.IsAllocated)
				_gchRenderFinished.Free();
			if(_binaryReader!=nullptr)
			{
				_binaryReader->Close();
				_binaryReader=nullptr;
			}
			delete _internalRenderNotifyFinished;
			delete _evRenderNotifyFinished;
